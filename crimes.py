...
# t = 1 if a burglary occurred in the current week and t = 0 if it did not.
...
for block in blocks:                        # make a matrix for each block
    ...
    while current_date < max(dates):        # begin at the beginning of time and end at the end of the sample
        crime_in_week = False               # Assume no crime
        for (count = 0, count < 7, count++):        # Go through 7 days to check the list for a crime.
            if block_crimes[0] == current_date:
                while block_crimes[0] == current_date:
    ...
    for week in enumerate(block_array):                 # Enumerate gives (index, value) tuple
        next_week = block_matrix.append(block_array[week[0]:])      # next week = [t, t+1, t+2...] where t is the column number
