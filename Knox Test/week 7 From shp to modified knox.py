...
os.chdir("C:/Users/titog/Desktop/city lab/knox test/files/")


# In[93]:

sf = shapefile.Reader("C:/Users/titog/Desktop/city lab/knox test/files/burglary2010")
...
sf2=shp2dataframe("C:/Users/titog/Desktop/city lab/knox test/files/burglary2010")
...
###from pandas to an array
sfa=np.array(sf2)
ta = np.array(sf2['time'])
xa= np.array(sf2['X_Coordina'])
ya= np.array(sf2['Y_Coordina'])
sc = np.array(sf2)
...
#knox( ca, ta, delta=300, tau=10, permutations=99, debug=False)
result = modified_knox(ca, ta, delta=2400, tau=30, permutations=99)
...
#subset, funciona!
#subset50 = sf2.loc[(sf2.Community_ == 50)]
#sf2['Community_'].head()
...
block = pd.unique(sf2['Block'].values.ravel())
...
####loop to generate subset per community area
...
for x in coma:
   ...
   #sc = np.array(sf2)
    ca = np.column_stack((xa,ya))
    result = modified_knox(ca, ta, delta, tau, permutations=99)
    print format(x) +",",("ca{0}".format(x)) +",", ("%2.8f"%result['pvalue'])+",",  (result['stat'])
...
pd.unique(sf2['Community_'].values.ravel())


# In[111]:

#sfa=np.array(d["ca50"])
    ta = np.array(d["ca50"]['time'])
    xa= np.array(d["ca50"]['X_Coordina'])
    ya= np.array(d["ca50"]['Y_Coordina'])
   #sc = np.array(sf2)
    ca = np.column_stack((xa,ya))
...
result = modified_knox(ca, ta, delta=600, tau=1, permutations=99)
